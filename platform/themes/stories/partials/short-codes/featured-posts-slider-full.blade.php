@php
    $featuredPosts = get_featured_posts(5, ['slugable', 'categories', 'categories.slugable']);
@endphp
                            ...
                            <div class="container">
                                <div class="entry-meta meta-0 font-small mb-20">
                                    @foreach($post->categories as $category)
                                        <a href="{{ $category->url }}" tabindex="{{ $loop->index }}"><span class="post-cat text-{{ ['warning', 'primary', 'info', 'success'][array_rand(['warning', 'primary', 'info', 'success'])] }} text-uppercase">{{ $category->name }}</span></a>
                                    @endforeach
                                </div>
